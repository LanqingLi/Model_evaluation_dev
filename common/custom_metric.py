...
class ClusteringMetric(EvalMetric):
    ...
    Relevant measures are adapted from http://scikit-learn.org/stable/modules/clustering.html#clustering-performance-evaluation
    ...
    def get_silhouette_score(self):
        print NotImplementedError("silhouette_score is defined for situation when the ground truth labels are not known and the distance between samples are explicitly defined for clustering")
        silhouette_score = []
        for label, pred in zip(self.labels_gt, self.labels_pred):
