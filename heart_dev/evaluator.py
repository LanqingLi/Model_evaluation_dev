...
class HeartPlaqueEvaluatorOffline(HeartPlaqueEvaluatorOnline):
    '''
    ...
    def __init__(self, data_dir, data_type, gt_dir,
        ...
        predict_df_list, gt_df_list = self.load_data()
        super(HeartPlaqueEvaluatorOffline, self).__init__(predict_df_list=predict_df_list,
                                                          gt_df_list=gt_df_list,
    ...
    def load_data(self):
        ...
        for PatientID in os.listdir(self.data_dir):
            if self.data_type == 'json':
                predict_json_dir = os.path.join(self.data_dir, PatientID)
                try:
                    predict_df_boxes = pd.read_json(predict_json_dir).T
                    predict_df_boxes = predict_df_boxes.rename(columns={'':'class'}
                except:
