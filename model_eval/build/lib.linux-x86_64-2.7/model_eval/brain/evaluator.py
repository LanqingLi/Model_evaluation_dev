...
class BrainSemanticSegEvaluatorOffline(object):
    '''
    ...
    def multi_class_evaluation(self):
        ...
        for thresh in self.conf_thresh:
            ...
            for predict_data, gt_nrrd, PatientID in zip(predict_data_list, gt_nrrd_list, self.patient_list):
                ...
                assert predict_data_cpy.shape[-1] == len(self.cls_name), 'the num of classes: %s in predict labels ' \
                    'should equal that defined in brain/classname_labelname_mapping.xls: %s' %(predict_data_cpy.shape[-1], len(self.cls_name))
            ...
            for cls_num in range(len(self.cls_name)):
                ...
                if self.cls_name[cls_num] not in self.opt_thresh:
                    ...
                    if self.result_df.iloc[-1][self.score_type] > self.opt_thresh[self.cls_name[cls_num]][self.score_type]:
                        self.opt_thresh[self.cls_name[cls_num]] = self.result_df.iloc[-1]
                        self.opt_thresh[self.cls_name[cls_num]].loc['threshold'] = thresh

        self.result_df = self.result_df.sort_values(['threshold', 'PatientID', 'class'])

        save_xlsx_json(self.result_df, self.opt_thresh, self.result_save_dir, self.xlsx_name, self.json_name, 'multi-class_evaluation', 'optimal_threshold')
        ...
        # self.result_df.to_excel(writer, 'binary-class_evaluation', index=False)
        ...
        # if os.path.isfile(
        #         os.path.join(self.result_save_dir, self.json_name + '_multi-class_evaluation.json')):
        #     os.remove(
        #         os.path.join(self.result_save_dir, self.json_name + '_multi-class_evaluation.json'))
        ...
        #
        # json_result_df = self.result_df.T.to_json()
        # with open(os.path.join(self.result_save_dir, self.json_name + '_multi-class_evaluation.json'),
        #           "w") as fp:
        #     json_result_df = json.loads(json_result_df, "utf-8")
        #     json.dump(json_result_df, fp)
        #
        # json_opt_thresh = opt_thresh.T.to_json()
        # with open(os.path.join(self.result_save_dir, self.json_name + '_optimal_threshold.json'),
        #           "w") as fp:
    ...
    def binary_class_evaluation(self):
        ...
        for thresh in self.conf_thresh:
            ...
            for predict_data, gt_nrrd, PatientID in zip(predict_data_list, gt_nrrd_list, self.patient_list):
                print ('processing PatientID: %s' %PatientID)
                assert predict_data.shape[1] == 2, 'the number of classes %s in predict labels should be 2 for binary classification' %(predict_data.shape[1])
                # transpose the predict_data to be shape = [512, 512, figure number]
                # one has to make a copy of part of predict_data, otherwise it will implicitly convert float to int
                predict_data_cpy = predict_data[:, 1, :, :].copy()
                predict_data_cpy = predict_data_cpy.transpose((1, 2, 0))
        ...
        save_xlsx_json(self.result_df, self.opt_thresh, self.result_save_dir, self.xlsx_name, self.json_name, 'binary-class_evaluation', 'optimal_threshold')
        # if not os.path.exists(self.result_save_dir):
        #     os.makedirs(self.result_save_dir)
        # print ("saving %s" % os.path.join(self.result_save_dir, self.xlsx_name))
        #
...
# class BrainInstanceSegEvaluatorOffline(object):
#
#     def __init__(self):
#
#     # 多分类分割模型评分,每次只选取单类别的检出框，把其余所有类别作为负样本。
#     def multi_class_evaluation(self):
#
#     def binary_class_evaluation(self):
