...
from common.xml_tools import *


def check_box_order(bndbox):
    if bndbox[2] < bndbox[0] or bndbox[3] < bndbox[1]:
        raise Exception("Boxes should be represented as [xmin, ymin, xmax, ymax], where xmax >= xmin and ymax >= ymin")

def iou(bndbox1, bndbox2):
    check_box_order(bndbox1)
    check_box_order(bndbox2)
    area1 = cal_box_square(bndbox1)
    area2 = cal_box_square(bndbox2)
    intersection_area = cal_boxes_intersection(bndbox1, bndbox2)
...
def calc_box_distance(box1, box2):
    ...
    :return:
    '''
    check_box_order(box1)
    check_box_order(box2)
...
def center_deviation(box1, box2):
    ...
    check_box_order(box1)
    check_box_order(box2)
    long_side = [float(max(box1[2] - box1[0], box2[2] - box2[0])), float(max(box1[3] - box1[1], box2[3] - box2[1]))]
