...
def find_objects_ensemble_stacking(bboxInfo_list, MODEL_WEIGHT_LIST, MODEL_CONF_LIST, Z_THRESHOLD, OBJ_THRESH=0.5,
                          SAME_BOX_THRESHOLD=np.array([1.6, 1.6]), SCORE_THRESHOLD=0.6, OBJECT_CLS_WEIGHTS={}, SAME_CLS_BOOST = 2.,
                          CLS_PRIORITY_DICT={"mass": 6,
                                            "calcific nodule": 5,
                                            "solid nodule": 4,
                                            "GGN": 3,
                                            "0-3nodule": 2,
    ...
    for curZ in sorted(objectSlices.keys()):
        ...
        for i in curBoxes_root:
            ...
            idx_count_dict = {model_idx:Boxes_union_model_idx.count(model_idx) for model_idx in range(len(bboxInfo_list))}
            bndbox_list = [j["bndbox"] for j in Boxes_union]
            class_list = [j["class"] for j in Boxes_union]
            weight_list = [j["prob"]/idx_count_dict[j["model_idx"]] for j in Boxes_union]
            merged_bndbox, merged_weight, merged_cls = anchor_metric.nms(bndbox_list=bndbox_list, weight_list=weight_list,
                                                                         cls_list=class_list, cls_priority_dict=CLS_PRIORITY_DICT)
            if merged_weight >= OBJ_THRESH:
                curBoxes_union.append({"matched": False,
                                       # add the id
                                       "boxID": i["boxID"],
                                       "model_idx": -1, # -1 stands for merged
                                       "bndbox": merged_bndbox,
                                       "class": merged_cls,
                                       "prob": merged_weight,
                                       "score": merged_weight * OBJECT_CLS_WEIGHTS[merged_cls]})
        ...
        for i in range(len(matched_object_list)):
            box_union = bboxInfo.loc[matchRes[matched_object_list[i]] - BOXID_VALUE]["unionFindSet"]
            ...
            objects[matched_object_list[i] - 1]["objectList"].append(matchRes[matched_object_list[i]] - BOXID_VALUE)
