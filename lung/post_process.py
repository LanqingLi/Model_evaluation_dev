            ...
            for i, row_gt in cls_gt_df.iterrows():
                ...
                for j, row_pred in cls_predict_df.iterrows():
                    if nodule_compare(predict_slices=pred_nodule[j][0], ground_truth_slices=gt_nodule[i][0],
                                      predict_bboxs=pred_nodule[j][1], ground_truth_bboxs=gt_nodule[i][1]):
                        cov_matrix[i , j] = 1
...
def nodule_compare(predict_slices, ground_truth_slices, predict_bboxs, ground_truth_bboxs):
    """
...
def cal_same_bbox(bbox_gt, bbox_pt, iou_thresh = config.IOU_THRESHOLD):
