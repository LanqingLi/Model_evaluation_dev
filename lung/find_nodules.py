...
def calcDICE(box1, box2):
    ...
    if np.any(box1[1] - box1[0] < 0) or np.any(box2[1] - box2[0] < 0):
        raise ValueError("Boxes should be represented as [xmin, ymin, xmax, ymax]. Box1: %s. Box2: %s. " % (str(box1), str(box2)))
    box1[1] += 1
    box2[1] += 1
...
def find_nodules(bboxInfo, Z_THRESHOLD = 3, SAME_BOX_THRESHOLD = 1., SCORE_THRESHOLD = 0.2):
    """将boundingbox转换为结节。
        ...
        if len(lst) > 1:
            for j1 in range(len(lst.index)):
                for j2 in range(j1+1, len(lst.index)):
                    if calcDICE(get_bounding_box_nparray(lst.iloc[j1]), get_bounding_box_nparray(lst.iloc[j2])) > SAME_BOX_THRESHOLD:
                        # 如果两个box相似程度高于SAME_BOX_THRESHOLD，那么认为这两个box表示同一个结节
    ...
    for curZ in sorted(noduleSlices.keys()):
        ...
        lastBoxes = [{"noduleID": k["id"], 
                     ...
                     for k in nodules if 0 < curZ - bboxInfo.loc[k["noduleList"][-1]]["instanceNumber"] <= Z_THRESHOLD[bboxInfo.loc[k["noduleList"][-1]]["nodule_class"]]]
