    ...
    for cls_predict_df, cls_gt_df in zip(cls_predict_df_list, cls_gt_df_list):
        ...
        # non-zero gt and predicted objects
            ...
            for i, row_pred in cls_predict_df.iterrows():
                predict_slices, predict_bboxs = object_slice_interpolate_pred(row_pred)
                pred_object.append((predict_slices, predict_bboxs))

            for j, row_gt in cls_gt_df.iterrows():
                gt_slices, gt_bboxs = object_slice_interpolate_gt(row_gt, z_threshold=1)
...
def object_slice_interpolate_pred(predict_df_record):
    """
    ...
    """
    predict_slices = range(predict_df_record['slice'][0],
                           predict_df_record['slice'][-1] + 1)
...
def object_slice_interpolate_gt(ground_truth_df_record, z_threshold=1):
    """
    :param ground_truth_df_record:一条ground truth记录，代表一个结节，DataFrame
    """
    ground_truth_slices = range(ground_truth_df_record['slice'][0] - z_threshold,
                                ground_truth_df_record['slice'][-1] + 1 + z_threshold)
